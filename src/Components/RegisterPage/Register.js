...
//   footer: '<a href="">Why do I have this issue?</a>'
       ...
       await axios.post("https://newqlinksbackapi.vercel.app/api/user/clientReg ", {name, email, password}).then((response)=>{
         hist('/dashboard')
         window.location.reload()
            ...
            swal(hist('/dashboard'));
             ...
             window.localStorage.setItem("dataUsers" ,JSON.stringify(response?.data?.data?.CreateUser) )
...
//   footer: '<a href="">Why do I have this issue?</a>'
       ...
       await axios.post("https://newqlinksbackapi.vercel.app/api/user/login", { email, password}).then((response)=>{
           window.localStorage.setItem("dataUsers" ,JSON.stringify(response.data.data))
           hist('/dashboard')
              window.location.reload()
               swal({
            title: " Success",
...
//   footer: '<a href="">Why do I have this issue?</a>'
  ...
  showLoaderOnConfirm: true,
    ...
    if (value === inputRef) {
       return axios.post("https://newqlinksbackapi.vercel.app/api/user/developerReg", {name, email, password}).then((response)=>{
           window.localStorage.setItem("dataUsers" ,JSON.stringify(response.data.data) )
           hist('/mainprofile')
           window.location.reload()
               swal({
...
//   footer: '<a href="">Why do I have this issue?</a>'
  ...
  return (
<div  style={{display : 'flex', justifyContent : "center", alignItems : "center", background : "rgba(0,0,0, 0.5)", minHeight : '90vh', position : 'fixed', width : '100vw'}} class="model">
{
    toggle ? 
    ...
    <div class="login-form default-form">
      <div class="form-inner">
        ...
        >
          ...
          <div class="form-group">
            <button onClick = {SignupDeveloper} class="theme-btn btn-style-one" type="submit" name="log-in">JOIN AS A DEVELOPER</button>
            <button onClick = {()=>{
                postData()

             
               
            }} style={{background : "#21CDBF"}} class="theme-btn btn-style-one" type="submit" >JOIN AS A CLIENT/CONTRACTOR</button>
          </div>
        </form>

        <div class="bottom-box">
          <div class="text">Already have an account? <a onClick={changeToggle} class="call-modal signup">Log In</a></div>
      ...
      <div class="form-inner">
        ...
        <form    onSubmit={(e)=>{
          ...
          <div class="form-group">
            ...
            }}class="theme-btn btn-style-one" type="submit" name="log-in">Log In</button>
          </div>
        </form>

        <div class="bottom-box">
          <div class="text">Don't have an account? <a onClick = {changeToggle} class="call-modal signup">Signup</a></div>
